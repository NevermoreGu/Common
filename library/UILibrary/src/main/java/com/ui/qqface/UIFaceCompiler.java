...
/**
 * {@link UIFaceView} 的内容解析器，将文本内容解析成 {@link UIFaceView} 想要的数据格式。
 *
 * @author cginechen
 * @date 2016-12-21
 */

public class UIFaceCompiler {
    private static final int SPAN_COLUMN = 2;

    private volatile static UIFaceCompiler sInstance;

    // cache
    private LruCache<CharSequence, ElementList> mCache;

    private IUIFaceManager mQQFaceManager;

    // 有多线程保护，如果内容非常多，可以扔到后台去（虽然暂时没有这个必要，但不避免有人想这么做）
    public static UIFaceCompiler getInstance(IUIFaceManager manager) {
        if (sInstance == null) {
            synchronized (UIFaceCompiler.class) {
                if (sInstance == null) {
                    sInstance = new UIFaceCompiler(manager);
                }
            }
        }
        return sInstance;
    ...
    private UIFaceCompiler(IUIFaceManager manager) {
