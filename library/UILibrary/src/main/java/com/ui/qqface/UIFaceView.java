...
import com.ui.R;
import com.ui.UILog;
import com.ui.link.ITouchableSpan;
...
public class UIFaceView extends View {
    private static final String TAG = "UIFaceView";
    private CharSequence mOriginText;
    private UIFaceCompiler.ElementList mElementList;
    private UIFaceCompiler mCompiler;
    ...
    public UIFaceView(Context context) {
        this(context, null);
    }

    public UIFaceView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.UIFaceStyle);
    }

    public UIFaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs,
                R.styleable.UIFaceView, defStyleAttr, 0);
        mQQFaceSizeAddon = -UIDisplayHelper.dp2px(context, 2); // 默认表情小一点好看
        mTextSize = array.getDimensionPixelSize(R.styleable.UIFaceView_android_textSize,
                UIDisplayHelper.dp2px(context, 14));
        mTextColor = array.getColor(R.styleable.UIFaceView_android_textColor, Color.BLACK);
        mIsSingleLine = array.getBoolean(R.styleable.UIFaceView_android_singleLine, false);
        mMaxLine = array.getInt(R.styleable.UIFaceView_android_maxLines, mMaxLine);
        int lineSpace = array.getDimensionPixelOffset(R.
                styleable.UIFaceView_android_lineSpacingExtra, 0);
        setLineSpace(lineSpace);
        int ellipsize = -1;
        ellipsize = array.getInt(R.styleable.UIFaceView_android_ellipsize, ellipsize);
        switch (ellipsize) {
            case 1:
        ...
        mMaxWidth = array.getDimensionPixelSize(R.styleable.UIFaceView_android_maxWidth, mMaxWidth);
        mSpecialDrawablePadding = array.getDimensionPixelSize(R.styleable.UIFaceView_ui_special_drawable_padding, 0);
        final String text = array.getString(R.styleable.UIFaceView_android_text);
        ...
        mMoreActionText = array.getString(R.styleable.UIFaceView_ui_more_action_text);
        mMoreActionColor = array.getColor(R.styleable.UIFaceView_ui_more_action_color, mTextColor);
    ...
    public void setCompiler(UIFaceCompiler compiler) {
        mCompiler = compiler;
    ...
    public void setText(CharSequence charSequence) {
        ...
        if (UILangHelper.isNullOrEmpty(mOriginText)) {
            if (!UILangHelper.isNullOrEmpty(oldText)) {
                mElementList = null;
                requestLayout();
                invalidate();
            }
        ...
        if (mOpenQQFace && mCompiler != null) {
            ...
            mElementList = new UIFaceCompiler.ElementList(0, mOriginText.length());
            String[] strings = mOriginText.toString().split("\\n");
            for (int i = 0; i < strings.length; i++) {
                mElementList.add(UIFaceCompiler.Element.createTextElement(strings[i]));
                if (i != strings.length - 1) {
                    mElementList.add(UIFaceCompiler.Element.createNextLineElement());
                }
            }
        }
        mNeedReCalculateLines = true;
    ...
    protected int calculateLinesAndContentWidth(int limitWidth) {
        ...
        List<UIFaceCompiler.Element> elements = mElementList.getElements();
        mSpanInfos.clear();
        mCurrentCalLine = 1;
    ...
    private void calculateLinesInner(List<UIFaceCompiler.Element> elements, int limitWidth) {
        UIFaceCompiler.Element element;
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (element.getType() == UIFaceCompiler.ElementType.DRAWABLE) {
            ...
            } else if (element.getType() == UIFaceCompiler.ElementType.TEXT) {
                CharSequence text = element.getText();
                measureText(text, widthStart, widthEnd);
            } else if (element.getType() == UIFaceCompiler.ElementType.SPAN) {
                UIFaceCompiler.ElementList spanElementList = element.getChildList();
            ...
            } else if (element.getType() == UIFaceCompiler.ElementType.NEXTLINE) {
                gotoCalNextLine(widthStart);
            } else if (element.getType() == UIFaceCompiler.ElementType.SPECIAL_BOUNDS_DRAWABLE) {
...
//            return 0;
//        }
//        List<UIFaceCompiler.Element> elementList = mElementList.getElements();
//        int paragraphCount = 0;
//        for(int i = 0; i < elementList.size(); i++){
//            UIFaceCompiler.Element element = elementList.get(i);
//            if(i == elementList.size() - 1){
//                if(element.getType() != UIFaceCompiler.ElementType.NEXTLINE){
//                    paragraphCount ++;
//                }
//            }else{
//                if(element.getType() == UIFaceCompiler.ElementType.NEXTLINE){
...
//    }
    ...
    private void drawElements(Canvas canvas, List<UIFaceCompiler.Element> elements, int usefulWidth) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            element = elements.get(i);
            UIFaceCompiler.ElementType type = element.getType();
            if (type == UIFaceCompiler.ElementType.DRAWABLE) {
                onDrawQQFace(canvas, element.getDrawableRes(), null, startLeft, endWidth, i == 0, i == elements.size() - 1);
            } else if (type == UIFaceCompiler.ElementType.SPECIAL_BOUNDS_DRAWABLE) {
                onDrawQQFace(canvas, 0, element.getSpecialBoundsDrawable(), startLeft, endWidth, i == 0, i == elements.size() - 1);
            } else if (type == UIFaceCompiler.ElementType.TEXT) {
                CharSequence text = element.getText();
            ...
            } else if (type == UIFaceCompiler.ElementType.SPAN) {
                UIFaceCompiler.ElementList spanElementList = element.getChildList();
            ...
            } else if (type == UIFaceCompiler.ElementType.NEXTLINE) {
    ...
    private class SpanInfo {
        ...
        public void onClick() {
            mTouchableSpan.onClick(UIFaceView.this);
        }
