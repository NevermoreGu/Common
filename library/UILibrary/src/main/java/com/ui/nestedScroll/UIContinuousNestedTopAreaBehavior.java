...
import static com.ui.UIInterpolatorStaticHolder.QUNITIC_INTERPOLATOR;
        ...
        if (target == child) {
            ...
            if (dyUnconsumed > 0) {
                ...
                if (bottomView != null) {
                    ...
                    if (bottomView.getBottom() - minBottom > dyUnconsumed) {
                        ...
                        if (canContentScroll) {
                            ((IUIContinuousNestedBottomView) bottomView).consumeScroll(dyUnconsumed - moveDistance);
                        }
            ...
            if (dyUnconsumed < 0) {
                if (child.getTop() <= dyUnconsumed) {
                    ...
                    int top = child.getTop();
                    setTopAndBottomOffset(0 - getLayoutTop());
                    int innerUnConsumed = ((IUIContinuousNestedTopView) child).consumeScroll(dyUnconsumed - top);
                    consumed[1] += dyUnconsumed - innerUnConsumed;
                } else if (child instanceof IUIContinuousNestedTopView) {
                    consumed[1] += dyUnconsumed - ((IUIContinuousNestedTopView) child).consumeScroll(dyUnconsumed);
                }
            }
        }
    }
