 ...
 * <li>每个 Tab 都可以非常灵活的配置，如果没有提供相关配置，则使用 UITabSegment 提供的配置，具体参考 {@link Tab}</li>
...
public class UITabSegment extends HorizontalScrollView {
    ...
    public UITabSegment(Context context, boolean hasIndicator) {
        this(context, null);
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDefaultSelectedColor = UIResHelper.getAttrColor(context, R.attr.ui_config_color_blue);
        mDefaultNormalColor = ContextCompat.getColor(context, R.color.ui_config_color_gray_5);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.UITabSegment, defStyleAttr, 0);
        mHasIndicator = array.getBoolean(R.styleable.UITabSegment_ui_tab_has_indicator, true);
        mIndicatorHeight = array.getDimensionPixelSize(R.styleable.UITabSegment_ui_tab_indicator_height,
                getResources().getDimensionPixelSize(R.dimen.ui_tab_segment_indicator_height));
        mTabTextSize = array.getDimensionPixelSize(R.styleable.UITabSegment_android_textSize,
                getResources().getDimensionPixelSize(R.dimen.ui_tab_segment_text_size));
        mIndicatorTop = array.getBoolean(R.styleable.UITabSegment_ui_tab_indicator_top, false);
        mDefaultTabIconPosition = array.getInt(R.styleable.UITabSegment_ui_tab_icon_position, ICON_POSITION_LEFT);
        mMode = array.getInt(R.styleable.UITabSegment_ui_tab_mode, MODE_FIXED);
        mItemSpaceInScrollMode = array.getDimensionPixelSize(R.styleable.UITabSegment_ui_tab_space, UIDisplayHelper.dp2px(context, 10));
        typefaceProviderName = array.getString(R.styleable.UITabSegment_ui_tab_typeface_provider);
        array.recycle();
    ...
    private void createTypefaceProvider(Context context, String className) {
        if (UILangHelper.isNullOrEmpty(className)) {
            return;
        }
    ...
    public UITabSegment addTab(Tab item) {
    ...
    public void selectTab(final int index, boolean noAnimation, boolean fromTabClick) {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                int preColor = UIColorHelper.computeColor(getTabSelectedColor(prevModel), getTabNormalColor(prevModel), animValue);
                int nowColor = UIColorHelper.computeColor(getTabNormalColor(nowModel), getTabSelectedColor(nowModel), animValue);
    ...
    public void updateIndicatorPosition(final int index, float offsetPercent) {
        ...
        int preColor = UIColorHelper.computeColor(getTabSelectedColor(preModel), getTabNormalColor(preModel), offsetPercent);
        int targetColor = UIColorHelper.computeColor(getTabNormalColor(targetModel), getTabSelectedColor(targetModel), offsetPercent);
    ...
    private int getTabSelectedTextSize(Tab item) {
        int textSize = item.getSelectedTextSize();
        if (textSize == Tab.USE_TAB_SEGMENT) {
            textSize = getTabTextSize(item);
        }
        return textSize;
    }

    private Drawable getTabSelectedBackground(Tab item) {
        Drawable res = item.getSelectedBackground();
        return res;
    }
    ...
    public static class Tab {
        ...
        public void setSelectedBackground(Drawable res) {
            this.selectedBackground = res;
        }
        ...
        private TextView ensureSignCountView(Context context) {
            if (mSignCountTextView == null) {
                mSignCountTextView = new TextView(context, null, R.attr.ui_tab_sign_count_view);
                RelativeLayout.LayoutParams signCountLp = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, UIResHelper.getAttrDimen(context, R.attr.ui_tab_sign_count_view_minSize));
                signCountLp.addRule(RelativeLayout.ALIGN_TOP, R.id.ui_tab_segment_item_id);
                signCountLp.addRule(RelativeLayout.RIGHT_OF, R.id.ui_tab_segment_item_id);
        ...
        public void showSignCountView(Context context, int count) {
            ...
            if (count != 0) {
                ...
                signCountLp.height = UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize_with_text);
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize_with_text));
                mSignCountTextView.setMinWidth(UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize_with_text));
                mSignCountTextView.setText(getNumberDigitsFormattingValue(count));
            } else {
                // 显示红点
                mSignCountTextView.setVisibility(View.GONE);
                signCountLp.height = UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize);
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize));
                mSignCountTextView.setMinWidth(UIResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.ui_tab_sign_count_view_minSize));
        ...
        public int getSignCount() {
            ...
            if (!UILangHelper.isNullOrEmpty(mSignCountTextView.getText())) {
        ...
        private String getNumberDigitsFormattingValue(int number) {
            if (UILangHelper.getNumberDigits(number) > mSignCountDigits) {
    ...
    public class TabAdapter extends UIItemViewsAdapter<Tab, TabItemView> {
        ...
        protected void bind(Tab item, TabItemView view, int position) {
            ...
            if (mCustomViews != null && mCustomViews.size() > 0) {
                view.setTag(R.id.ui_view_can_not_cache_tag, true);
    ...
    public class TabItemView extends RelativeLayout {
        ...
        public TabItemView(Context context) {
            ...
            // 用于提供给customView布局用
            mTextView.setId(R.id.ui_tab_segment_item_id);
        ...
        public void setColorInTransition(Tab tab, int color) {
            ...
            if (tab.isDynamicChangeIconColor()) {
                ...
                if (icon != null) {
                    UIDrawableHelper.setDrawableTintColor(icon, color);
        ...
        public void updateDecoration(Tab tab, boolean isSelected) {
            ...
            int size = isSelected ? getTabSelectedTextSize(tab) : getTabTextSize(tab);
            mTextView.setTextColor(color);
            mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, size);

            Drawable background = getTabSelectedBackground(tab);
            ...
            if (icon == null) {
                ...
                mTextView.setCompoundDrawablePadding(UIDisplayHelper.dp2px(getContext(), 2));
                setDrawable(mTextView, icon, getTabIconPosition(tab));
            }
        }
    }
