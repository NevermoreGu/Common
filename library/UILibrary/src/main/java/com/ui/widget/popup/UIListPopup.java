...
/**
 * 继承自 {@link UIPopup}，在 {@link UIPopup} 的基础上，支持显示一个列表。
 *
 * @author cginechen
 * @date 2016-11-16
 */
...
public class UIListPopup extends UIPopup {
     ...
     * @param direction Popup 的方向，为 {@link UIPopup#DIRECTION_NONE}, {@link UIPopup#DIRECTION_TOP} 和 {@link UIPopup#DIRECTION_BOTTOM} 中的其中一个值。
     * @param adapter   列表的 Adapter
     */
    public UIListPopup(Context context, @Direction int direction, BaseAdapter adapter) {
        super(context, direction);
        mAdapter = adapter;
    }

    public void create(int width, int maxHeight, AdapterView.OnItemClickListener onItemClickListener) {
        ListView listView = new UIWrapContentListView(mContext, maxHeight);
