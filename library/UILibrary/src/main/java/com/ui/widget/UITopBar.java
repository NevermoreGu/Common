...
import com.ui.R;
import com.ui.alpha.UIAlphaImageButton;
...
public class UITopBar extends RelativeLayout {
    ...
    // 这个构造器只用于UI内部，不开放给外面用，目前用于UITopBarLayout
    UITopBar(Context context, boolean inTopBarLayout) {
        super(context);
        initVar();
        if (inTopBarLayout) {
            int transparentColor = ContextCompat.getColor(context, R.color.ui_config_color_transparent);
            mTopBarSeparatorColor = transparentColor;
            mTopBarSeparatorHeight = 0;
            mTopBarBgColor = transparentColor;
        } else {
            init(context, null, R.attr.UITopBarStyle);
        }
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTopBarSeparatorHeight = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_separator_height, 1);
    ...
    void getCommonFieldFormTypedArray(Context context, TypedArray array) {
        mLeftBackDrawableRes = array.getResourceId(R.styleable.UITopBar_ui_topbar_left_back_drawable_id, R.id.ui_topbar_item_left_back);
            ...
            if ((mTitleGravity & Gravity.HORIZONTAL_GRAVITY_MASK) == Gravity.CENTER_HORIZONTAL) {
                ...
                if (mLeftViewList.isEmpty()) {
                    ...
                    titleContainerViewLeft += UIResHelper.getAttrDimen(getContext(),
                            R.attr.ui_topbar_title_margin_horizontal_when_no_btn_aside);
                }
            }
